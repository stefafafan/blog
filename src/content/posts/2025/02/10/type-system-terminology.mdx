...
上記の例では、 `Animal` というインターフェイスを定義し、 `Dog` と `Cat` はそれぞれ `Animal` インターフェイスに構造が合う形で実装されています。そのため、 `Dog` と `Cat` は `Animal` の部分型として扱われ、 `talk` 関数に引数として渡すことができます。
...
Subtyping に関する概念として Variance（変性）があります。変性とは上位型と下位型の関係性において、どのような性質を持つかを表す概念です。
...
以下の例のようにプリミティブ型をそのまま利用する際は Rust のコンパイラは型エラーを出力しません。このようなケースにおいては後述する New Type Pattern などを利用して型安全性を確保することが推奨されています。
...
- Type Safety = Type Soundness = 型安全性
- Structural = 構造的
